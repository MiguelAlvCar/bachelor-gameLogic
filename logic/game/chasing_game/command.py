...
def command(chasing_game: ChasingGameBase, unit_index: int, command_index: CommandType, is_red_command: bool) -> tuple[npt.NDArray[np.int16], npt.NDArray[np.int16]]:
    ...
    if position_contrary_units[target_field[0], target_field[1]] != -1:
        edited_red_units, edited_blue_units = combat(chasing_game, unit_index, position_contrary_units[target_field[0], target_field[1]][0], is_red_command)
    else:
